function getStatusText(statusCode, approver) {
    const statusMap = {
        ...
        6: `<div style='color: darkcyan'>Instructor requested to edit the grades. Request approved by <u>${approver}</u>. <sup>[${statusCode}]</sup></div>`,
...
function fetchData(type, params = {}) {
    return $.ajax({
        type: 'GET',
        url: '../../model/forms/grade-history/grade-history-controller.php',
        data: { type, ...params }
    });
}
            ...
            let table_builder = ret.length 
                ? ret.map(item => `
                    <tr>
                        ...
                        <td>${item.UNIT}</td>
                        <td class="text-start">${cleanScheduleStr(item.SCHEDULE)}</td>
                        <td class="text-start">${item.INSTRUCTOR}</td>
                        <td class="text-center"><button class="btn btn-primary btnView" data-bs-toggle="modal" data-bs-target="#recordHistoryModal" style="font-size: 12px;" value='${item.OFF_ID}'>View</button></td>
                    </tr>`).join('')
...
$(document).on("click", ".btnView", async function () {
    ...
    let table_builder = ret.length 
        ...
        : "<tr class='text-center'><td colspan='3'>No Submission History Found.</td></tr>";
