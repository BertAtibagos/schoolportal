<?php 
    ...
    function buildTable(array $data): array {
        ...
        foreach ($data as $row) {
            $html .= "<tr>
                        ...
                        <td class='ps-4' style='width: 15%'>" . $row['NUMBER'] . "</td>
    ...
    $id          = (int) ($_SESSION['USERID'] ?? 0);
    $deptid      = $_SESSION['USERDEPT'] ?? '';
    ...
    $queries = [
        ...
        "COURSE" => [
            "query" => "SELECT DISTINCT info.`SchlAcadCrseSms_ID` `ID`,
                        ...
                        ORDER BY `DESC` ASC;",
            "data_types" => "iiii",
            "param" => [$deptid, $levelid, $yearid, $periodid],
        ...
        "YEARLEVEL" => [
            "query" => "SELECT DISTINCT info.`SchlAcadYrLvlSms_ID` `ID`,
                        ...
                        ORDER BY `DESC` ASC;",
            "data_types" => "isss" . ($types_course ?? ''),
            "param" => [$deptid, $levelid, $yearid, $periodid, ...$courseid],
        ...
        "SECTION" => [
            "query" => "SELECT DISTINCT info.`SchlAcadSecSms_ID` `ID`,
                        ...
                        ORDER BY `DESC` ASC;",
            "data_types" => "isss" . ($types_course ?? '') . ($types_yearlevel ?? ''),
            "param" => [$deptid, $levelid, $yearid, $periodid, ...$courseid, ...$yearlevelid],
