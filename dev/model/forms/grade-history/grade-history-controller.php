<?php
	...
	if(isset($_GET['type'])){
        if($_GET['type'] == 'ACADLEVEL'){
			...
			$qry = "SELECT IFNULL(yrlvl.`SchlAcadYrLvlSms_ID`, 0) `ID`,
                                ...
                                FROM (
                                        ...
                                        AND off.`SchlAcadLvl_ID` = ".$_GET['levelid']."
                                        AND off.`SchlAcadYr_ID` = ".$_GET['yearid']."
                                        AND off.`SchlAcadPrd_ID` = ".$_GET['periodid']."
                                ) AS tbl
			...
			$qry = "SELECT IFNULL(crse.`SchlAcadCrseSms_ID`, 0) `ID`,
                                ...
                                FROM (
                                        ...
                                        AND off.`SchlAcadYr_ID` = ".$_GET['yearid']."
                                        AND off.`SchlAcadPrd_ID` = ".$_GET['periodid']."
                                        AND off.`SchlAcadYrLvl_ID` = ".$_GET['yearlevelid']."
                                ) AS tbl
			...
			$qry = "SELECT IFNULL(sec.`SchlAcadSecSms_ID`, 0) `ID`,
                                ...
                                FROM (
                                        ...
                                        AND off.`SchlAcadPrd_ID` = ".$_GET['periodid']."
                                        AND off.`SchlAcadYrLvl_ID` = ".$_GET['yearlevelid']."
                                        AND off.`SchlAcadCrses_ID` = ".$_GET['courseid']."
                                ) AS tbl
		...
		} else if ($_GET['type'] == 'SUBJECT_LIST'){
			$qry = "SELECT tbl.`SchlEnrollSubjOffSms_ID` `OFF_ID`,
                                ...
                                FROM (SELECT off.`SchlEnrollSubjOffSms_ID`,
                                        ...
                                        AND off.`SchlAcadYrLvl_ID` = ".$_GET['yearlevelid']."
                                        AND off.`SchlAcadCrses_ID` = ".$_GET['courseid']."
                                        AND off.`SchlAcadSec_ID` = ".$_GET['sectionid']."
                                ) AS tbl
			...
			$qry = "SELECT his.`SchlStudAcadRecAppHis_ISAPPROVED` `ISAPPOVED`,
                                ...
                                AND his.`SchlEnrollSubjOff_ID` = ".$_GET['subjid']."
			";
			$rsreg = $dbConn->query($qry);
			$fetch = $rsreg->fetch_ALL(MYSQLI_ASSOC);
