<?php
	...
	if ($_GET['type'] == 'ACADLEVEL'){
					...
					FROM (SELECT DISTINCT `subj_off`.`SchlAcadLvl_ID` `HIS_LVL_ID`,
						...
						`yr`.`SchlAcadYr_RANKNO` `HIS_YR_RANK_NO`
								...
								WHERE `his`.`SchlStudAcadRecAppHis_STATUS` = 1
									...
									AND `his`.`SchlSign_UserID`=  ". $_SESSION['USERID']." 
					  UNION 
						...
						`yr`.`SchlAcadYr_RANKNO` `YR_RANK_NO`
						  ...
						  LEFT JOIN `schoolenrollmentsubjectoffered` `subj_off`
								...
								WHERE `subj_off`.`SchlEnrollSubjOff_STATUS` = 1
									...
									AND `studrec`.`SchlSign_ID`= ". $_SESSION['USERID'].") AS p
						...
						`yr`.`SchlAcadYr_RANKNO` `YR_RANK_NO`
						  ...
						  LEFT JOIN `schoolenrollmentsubjectoffered` `subj_off`
								...
								WHERE `subj_off`.`SchlEnrollSubjOff_STATUS` = 1
									...
									AND `subj_off`.`SchlAcadLvl_ID` = " .$_GET['levelid'] .") AS p
						...
						`prd`.`SchlAcadPrd_RANKNO` `PRD_RANK_NO`
						  ...
						  LEFT JOIN `schoolenrollmentsubjectoffered` `subj_off`
								...
								WHERE `subj_off`.`SchlEnrollSubjOff_STATUS` = 1
									...
									AND `studrec`.`SchlSign_ID`= ". $_SESSION['USERID']." 
									AND `subj_off`.`SchlAcadLvl_ID` = " .$_GET['levelid'] ." 
									AND `subj_off`.`SchlAcadYr_ID` = " .$_GET['yearid'] .") AS p
		...
		$dbConn->close();
		echo json_encode($fetch);
	} else if ($_GET['type'] == 'FOR_APPROVAL_REQUEST_LIST'){
		...
		echo json_encode($fetch);
				...
				$dbConn->close();
				echo json_encode($fetch);
	} else if ($_GET['type'] == 'MANAGE_SUBMITTED_REQUEST'){
				...
				//clearstatcache();
				echo $status;
	} else if ($_GET['type'] == 'GET_SUBMITTED_REQUEST_HISTORY'){
		$qry = "SELECT `his`.`SchlStudAcadRecAppHis_DATETIME` `DATE`,
				...
				WHERE `his`.`SchlStudAcadRecAppHis_STATUS` = 1
					AND `his`.`SchlStudAcadRecAppHis_ISACTIVE` = 1
					AND `his`.`SchlStudAcadRecAppHis_ISAPPROVED` = " .$_GET['isapproved'] ."
					AND `his`.`SchlSign_UserID`=  ". $_SESSION['USERID']."
					AND `subj_off`.`SchlAcadLvl_ID` = " .$_GET['levelid'] ."
					AND `subj_off`.`SchlAcadYr_ID` = " .$_GET['yearid']."
					AND `subj_off`.`SchlAcadPrd_ID` = " .$_GET['periodid'];
