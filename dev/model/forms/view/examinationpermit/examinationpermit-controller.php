<?php
	...
	if (isset($_GET['action']) && isset($_GET['type']))
	{
		if ($_GET['type'] == 'ACADLEVEL'){
			...
			$rsreg = $dbConn->query($qry);
			$fetch = $rsreg->fetch_ALL(MYSQLI_ASSOC);
		} else if ($_GET['type'] == 'ACADEXAMPERIOD'){
			//$qry = "CALL spGETacademicexaminationperiod(".$_GET['levelid'] .",".$_GET['yearid'] .",".$_GET['periodid'].")";
			//$rsreg = $dbConn->query($qry);
			//$fetch = $rsreg->fetch_ALL(MYSQLI_ASSOC);
			$qry = "SELECT IFNULL(`SchlAcadExamPrd_CODE`,'') `CODE`,
					...
					WHERE `SchlAcadExamPrd_STATUS` = 1
						...
						AND `SchlAcadLvl_ID` = " . intval($_GET['levelid']) . "
						AND `SchlAcadYr_ID` = " . intval($_GET['yearid']) . "
						AND `SchlAcadPrd_ID` = " . intval($_GET['periodid']) . "
			...
			$qry = "SELECT IFNULL(`ass`.`SchlStud_ID`,0) `ID`,
							...
							(SELECT CASE COUNT(`SchlEnrollAssSms_ID`)
							  ...
							  FROM `schoolenrollmentassessment` 
								WHERE `SchlEnrollAssSms_ID` = `ass`.`SchlEnrollAssSms_ID`
									...
									LIKE CONCAT('%,'," . $_GET['subjofferedid'] . ",',%')
							) `STATUS`,
						...
						WHERE CONCAT(',',`ass`.`SchlAcadSubj_ID`,',') LIKE CONCAT('%,'," . $_GET['subjofferedid'] . ",',%') 
							AND `ass`.`schlacadlvl_id`=  " . intval($_GET['levelid']) . "
							AND `ass`.`schlacadyr_id`= " . intval($_GET['yearid']) . "
							AND `ass`.`SchlAcadPrd_ID`= " . intval($_GET['periodid']) . "
